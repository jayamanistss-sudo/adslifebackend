...
export class MonitoringService {
  ...
  async blockIp(ip: string, reason: string, blockedBy: number, expiresAt?: Date) {
    await this.db.query(
      `INSERT INTO blocked_ips (ip_address,reason,blocked_by,expires_at) VALUES (?,?,?,?)
  ...
  async unblockIp(ip: string) {
    const result = await this.db.query('DELETE FROM blocked_ips WHERE ip_address = ?', [ip]);
  ...
  async recentUnauthorizedCount(ip: string, minutes: number): Promise<number> {
    try {
      const [{ cnt }] = await this.db.query(
        `SELECT COUNT(*) as cnt FROM api_logs
         WHERE ip_address=? AND status_code IN (401,403)
         AND created_at >= DATE_SUB(NOW(), INTERVAL ? MINUTE)`,
        [ip, minutes],
      );
      return +cnt;
  ...
  async getOverview() {
    ...
    const [
      ...
      this.db.query(`SELECT COUNT(*) as cnt FROM api_logs WHERE DATE(created_at)=?`, [today]),
      this.db.query(`SELECT COUNT(DISTINCT user_id) as cnt FROM api_logs WHERE DATE(created_at)=? AND user_id IS NOT NULL`, [today]),
      this.db.query(`SELECT COUNT(*) as cnt FROM auth_logs WHERE action='login_failure' AND DATE(created_at)=?`, [today]),
      this.db.query(`SELECT COUNT(*) as cnt FROM api_logs WHERE status_code BETWEEN 400 AND 499 AND DATE(created_at)=?`, [today]),
      this.db.query(`SELECT COUNT(*) as cnt FROM api_logs WHERE status_code>=500 AND DATE(created_at)=?`, [today]),
      this.db.query(`SELECT ROUND(AVG(response_time_ms),0) as avg FROM api_logs WHERE DATE(created_at)=?`, [today]),
      this.db.query(`SELECT endpoint, COUNT(*) as count FROM api_logs WHERE DATE(created_at)=? GROUP BY endpoint ORDER BY count DESC LIMIT 10`, [today]),
      this.db.query(`SELECT ip_address, COUNT(*) as count FROM api_logs WHERE DATE(created_at)=? GROUP BY ip_address ORDER BY count DESC LIMIT 10`, [today]),
      this.db.query(`SELECT COUNT(*) as cnt FROM security_events WHERE is_resolved=0 AND DATE(created_at)=?`, [today]),
      this.db.query(`SELECT COUNT(*) as cnt FROM alert_logs WHERE is_read=0`),
      this.db.query(`SELECT HOUR(created_at) as hr, COUNT(*) as count FROM api_logs WHERE DATE(created_at)=? GROUP BY HOUR(created_at) ORDER BY hr`, [today]),
  ...
  async getApiLogs(q: any) {
    return this._paginate('api_logs', q, [
      ...
      ['suspicious_only', 'is_suspicious = 1', null, true],
  ...
  async getAlerts(q: any) {
    const result = await this._paginate('alert_logs', q, [
      ['severity', 'severity = ?'],
      ['search', '(title LIKE ? OR message LIKE ?)', (v: string) => [`%${v}%`, `%${v}%`]],
    ]);
    setImmediate(() => this.db.query('UPDATE alert_logs SET is_read=1 WHERE is_read=0').catch(() => {}));
    return result;
  }
  ...
  async resolveSecurityEvent(id: number, adminId: number) {
    const result = await this.db.query(
      'UPDATE security_events SET is_resolved=1, resolved_at=NOW(), resolved_by=? WHERE id=?',
      [adminId, id],
    );
    return result.affectedRows > 0;
  }
  ...
  private async _paginate(table: string, q: any, filters: any[]) {
    ...
    for (const [key, clause, transform, flag] of filters) {
      const val = q[key];
      if (val === undefined || val === null || val === '' || val === false) continue;
      if (flag) {
        if (val === true || val === 'true') where.push(clause as string);
        continue;
      }
      if (!transform) {
        where.push(clause as string);
        params.push(val);
      } else {
    ...
    const data = await this.db.query(
      `SELECT * FROM ${table} ${whereStr} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
